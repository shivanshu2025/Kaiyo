...
import {
  solutionCategories as fallbackCategories,
  slugifySolutionLabel,
...
export default function SolutionsDropdown() {
  ...
  const activeCategory = categories.find((cat) =>
    cat.items.some((item) => {
      const slug = slugifySolutionLabel(item.label);
      return pathname === item.href || pathname === `/solutions/${slug}`;
    })
  )?.title ?? 'Solutions';
