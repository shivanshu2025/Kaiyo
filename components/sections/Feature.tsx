...
type FeatureData = { title: string; description: string; icon: string; order: number };

interface Props {
  dynamicFeatures?: FeatureData[];
}

export default function OurStorySection({ dynamicFeatures }: Props) {
    ...
    <section className="relative flex w-full flex-col items-center justify-start overflow-hidden bg-[#E9E9E7] px-4 py-16 sm:py-20 md:py-28 font-serif text-[#2D3627] sm:px-6 md:px-8">
          ...
          <div className="group relative flex h-full w-full cursor-pointer items-center justify-center rounded-full border-2 sm:border-2 md:border-4 border-white/20 p-1 shadow-2xl">
            ...
            <svg viewBox="0 0 100 100" className="h-full w-full animate-[spin_15s_linear_infinite]">
              <path
                ...
                fill="transparent"
              />
              <text className="fill-white text-[9px] font-bold uppercase tracking-[2px] md:text-[10px]">
      ...
      {dynamicFeatures && dynamicFeatures.length > 0 && (
        <motion.div
          ...
          className="relative z-10 mt-16 grid w-full max-w-6xl gap-6 px-2 sm:grid-cols-2 sm:px-0 lg:grid-cols-3"
        >
          {dynamicFeatures
            ...
            .map((feature, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="group rounded-2xl bg-white/60 p-6 backdrop-blur-sm transition-all duration-300 hover:bg-white hover:shadow-xl"
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#2D3627] text-white">
                  <span className="text-lg font-bold">{feature.icon || String.fromCharCode(65 + i)}</span>
                </div>
                <h3 className="mb-2 text-lg font-bold text-[#2D3627]">{feature.title}</h3>
